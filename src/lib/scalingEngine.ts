...
export function systemAudit(): { findings: AuditFinding[]; criticalFailures: AuditFinding[]; remediation: AuditFinding[] } {
  ...
  const findings: AuditFinding[] = [
    ...
    { area: "pilot_release", pass: gate.pass !== false, severity: gate.pass === false ? "medium" : "none", detail: gate.pass === false ? "Pilot gate holds open items pending review." : "Pilot gate satisfied for Phase B." },
    { area: "translation", pass: TRANSLATION_JOBS.every((j) => !translationPublishable(j).publishable ? true : !!j.approvedBy), severity: "medium", detail: "Hindi translation is in review; no translation is published yet.", remediation: "Complete Hindi technical and safety term review, then UAT." },
