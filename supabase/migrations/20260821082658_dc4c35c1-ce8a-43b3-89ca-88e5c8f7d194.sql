CREATE TABLE IF NOT EXISTS public.authorization_test_runs (
...
GRANT SELECT ON public.authorization_test_runs TO authenticated;
GRANT ALL ON public.authorization_test_runs TO service_role;

ALTER TABLE public.authorization_test_runs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Platform administrators read authorization test runs" ON public.authorization_test_runs;
CREATE POLICY "Platform administrators read authorization test runs"
  ON public.authorization_test_runs FOR SELECT TO authenticated
  USING (public.is_platform_admin(auth.uid()));

DROP TRIGGER IF EXISTS trg_authorization_test_runs_updated ON public.authorization_test_runs;
CREATE TRIGGER trg_authorization_test_runs_updated
  BEFORE UPDATE ON public.authorization_test_runs
...
BEGIN
  ...
  BEGIN
    ...
    ------------------------------------------ approval vs publication rights
    ...
    -- Discard every temporary record created above.
