...
-- roles and temporary product fixtures, execute the real approval and
-- publication functions and real SELECT / UPDATE statements under the
-- real row-level security policies as each role, then discard every
-- test record by rolling the inner block back.
--
-- Only the pass/fail summary is kept, in public.authorization_test_runs.
-- Inspect the latest run with:
--   select total, passed, failed, aborted_with, results
--   from public.authorization_test_runs order by run_at desc limit 1;
--
  ...
  EXECUTE $f$
    ...
    BEGIN
      ...
      INSERT INTO public.product_source_documents(product_id, product_version_id, source_document_id,
        ...
        VALUES (pid, vid, did, 'manufacturer_tds', 'verified');
